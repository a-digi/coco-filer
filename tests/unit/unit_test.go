...
func TestFileManager_MoveFile(t *testing.T) {
	dir := t.TempDir()
	fm := filer.NewFileManager(dir)

	srcFile := filepath.Join(dir, "test.txt")
	dstFile := filepath.Join(dir, "subdir", "test.txt")
...
func TestFileMultiPartManager_MoveFile(t *testing.T) {
	dir := t.TempDir()
	fm := filer.NewFileMultiPartManager(dir)
	data := "test multipart content"
	file := &inMemoryMultipartFile{Reader: strings.NewReader(data)}
	defer file.Close()

	// Create a fake multipart.FileHeader
